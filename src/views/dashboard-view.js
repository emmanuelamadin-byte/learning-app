  ...
  return html`
    <div className="grid gap-4 sm:gap-6">
      <section className="grid gap-4 sm:gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="glass-panel relative overflow-hidden rounded-[1.5rem] sm:rounded-[2rem] p-5 sm:p-8">
          <div className="hero-orb hero-orb--green -left-6 top-6"></div>
          <div className="relative">
            <div className="text-[10px] sm:text-xs uppercase tracking-[0.26em] sm:tracking-[0.3em] text-white/45">
              Personal dashboard
            </div>
            <h1 className="mt-3 sm:mt-4 max-w-xl font-display text-3xl sm:text-4xl md:text-5xl font-bold leading-tight text-white">
              Stay visible. Stay consistent. Stack proof every day.
            </h1>
            <p className="mt-3 sm:mt-4 max-w-2xl text-sm sm:text-base leading-6 sm:leading-7 text-white/62">
            ...
            <div className="mt-6 sm:mt-8 flex flex-col gap-5 sm:gap-6 lg:flex-row lg:items-center">
              ...
              <div className="grid gap-3 w-full">
                ${currentMetrics.insights.map(
                  (insight) => html`
                    <div
                      key=${insight}
                      className="rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3 text-sm leading-6 text-white/70"
                    >
                      ${insight}
                    </div>
                  `,
        ...
        <form className="glass-panel rounded-[1.5rem] sm:rounded-[2rem] p-5 sm:p-8" onSubmit=${onCreateSession} noValidate>
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-[10px] sm:text-xs uppercase tracking-[0.24em] sm:tracking-[0.28em] text-white/45">
                Daily logging
              </div>
              <h2 className="mt-2 sm:mt-3 font-display text-2xl sm:text-3xl font-semibold text-white">
                Log today’s work
              </h2>
            </div>
            <span className="hidden sm:inline-flex rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs uppercase tracking-[0.22em] text-white/45">
              Auto timestamp
            </span>
          </div>
          ...
          <div className="mt-5 sm:mt-6 grid gap-4">
            ...
            <button
              type="submit"
              className="brand-gradient-bg min-h-12 rounded-2xl px-4 text-sm font-semibold"
            >
              Log session
            </button>
          </div>
      ...
      <section className="grid gap-4 sm:gap-6 lg:grid-cols-[0.38fr_0.62fr]">
        <div className="glass-panel rounded-[1.5rem] sm:rounded-[2rem] p-5">
          <div className="text-sm font-semibold text-white">Filter session history</div>
          <div className="mt-1 text-sm text-white/50">
            Narrow your log by day or by minimum hours.
          </div>
